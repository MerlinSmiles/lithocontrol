...
class MeasureTask(Task):
    def __init__(self, settings):
        ...
        self.settings['acq_rate'] = 200000          # samples/second
    ...
    def EveryNCallback(self):
        read = int32()
        data = zeros(4*self.num_chans*self.acq_samples)
        if self.running:
            ...
            self.ReadAnalogF64(self.acq_samples,
                               10.0,
                               DAQmx_Val_GroupByChannel,
                               data,
                               4*self.num_chans*self.acq_samples,
                               byref(read),
                               None)
            meas_data = np.reshape(data[:read.value*self.num_chans],(self.num_chans,-1))
        else:
            self.running = True
            self.ReadAnalogF64(self.acq_samples-500,
                               10.0,
                               DAQmx_Val_GroupByChannel,
                               data,
                               4*self.num_chans*self.acq_samples,
        ...
        # pointstep = 1
        # # sample spacing
        # T = 1.0*pointstep / self.settings['acq_rate']

        # for i in self.ch_in_list:
        #     if self.settings['in'][i]['lockin']:
        #         if self.settings['buff'].get_partial().size > self.num_chans*N:
        #             # x = np.linspace(0.0, N*T, N/pointstep)
        #             y = self.settings['buff'].get_partial()[i,-(N+1)::pointstep]
        #             # y = np.sin(50.0 * 2.0*np.pi*x) + 0.5*np.sin(80.0 * 2.0*np.pi*x)
        #             # yf = scipy.fftpack.rfft(y)
        #             # xf = np.linspace(0.0, 1.0/(2.0*T), N)

        #             yf = np.fft.fft(y)
        #             xf = np.fft.fftfreq(len(yf), d=T)

        #             self.settings['in'][i]['fft_arr'] = np.array([xf,2.0/N * np.abs(yf)])
...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ch0['channel'] = "ai0"
        ch0['min'] = -0.1 # +/- 100 mV is the minimum bipolar range
        ch0['max'] = 0.1
        ch0['lockin'] = True
        ch0['plot_raw'] = True
        ch0['li_tc'] = 0.1
        ch0['li_buffer'] = ringbuffer.RingBuffer((4, self.settings['buffer_size']/1000))
        ...
        ch1['plot_raw'] = True
        ch1['li_tc'] = 0.1
        ch1['li_buffer'] = ringbuffer.RingBuffer((4, self.settings['buffer_size']/1000))
        ...
        # ch2['plot_raw'] = False
        # ch2['li_tc'] = 0.1
        # ch2['li_buffer'] = ringbuffer.RingBuffer((3, self.settings['buffer_size']/1000))
        # ch2['fft_arr'] = np.array([])
        # ch2['freq_chan'] = 0
        # ch2['curr_amp'] = ch1['curr_amp']
        ...
        # ch3['plot_raw'] = False
        # ch3['li_tc'] = 0.1
        # ch3['li_buffer'] = ringbuffer.RingBuffer((3, self.settings['buffer_size']/1000))
        ...
        self.cAmpSpinBox.setValue(self.settings['in'][1]['curr_amp'])
            ...
            if raw_buffer.size >0:
                for n in range(len(self.settings['in'].keys())):
                    if self.settings['in'][n]['plot_raw']:
                        # for n,i in enumerate(raw_buffer):
                        # name=str(np.average(raw_buffer[n][-100:]))
                        self.plotlist2[n]['plot'].setData(y=raw_buffer[n], name=str(1))
                        self.plotlist2[n]['plot'].setPen(color=self.kelly_colors[self.colors[n]])

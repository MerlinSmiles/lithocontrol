...
class MyApp(QtGui.QWidget):
 ...
 def test(self):
  ...
  self.threadPool.append( WorkThread(6) )
  self.connect( self.threadPool[len(self.threadPool)-1], QtCore.SIGNAL("update(QString)"), self.add )
  self.connect( self, QtCore.SIGNAL("add(QString)"), self.data )
...
class WorkThread(QtCore.QThread):
 ...
 def run(self):
  for i in range(6):
   ...
   self.emit( QtCore.SIGNAL('add(PyQt_PyObject)'), np.random.random(self.num) )
  return
